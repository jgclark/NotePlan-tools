...
# by Jonathan Clark, v0.2, 23.1.2021
#-------------------------------------------------------------------------------
VERSION = "0.2.0"
require 'date'
require 'cgi'
...
# January 30, 2021 at 11:11PM | Espen Eriksen Trio | In the Mountains | Never Ending January | https://ift.tt/2TRqQiB | https://ift.tt/2LptJng
# January 30, 2021 at 11:56AM | Brian Doerksen | Creation Calls | Today | https://ift.tt/2qQI2Sq | https://ift.tt/3pYs1by
# END_S_DATA

# instapaper_test_data = <<-END_I_DATA
# January 30, 2021 at 05:49AM \\ Thomas Creedy: Imago Dei \\ https://ift.tt/3rJl1Bh \\ 
# January 30, 2021 at 06:02AM \\ Is the 'seal of the confessional' Anglican? \\ https://ift.tt/2MpMAPX \\ "Andrew Atherstone writes: The Church of England has at last published the report of the ‘Seal of the Confessional’ working party , more than a year after it…"
...
class NPCalFile
  ...
  def insert_new_line(new_line, line_number)
    ...
    puts '  insert_new_line ...' if $verbose
  ...
  def append_media_header()
    ...
    # @lines[n + 1] = '### Media'
    # @line_count += 1
    @media_header_line = @line_count - 1
  end
...
spotify_filepath = IFTTT_FILEPATH+SPOTIFY_FILE
  ...
  elsif File.exists?(spotify_filepath)
    if File.empty?(spotify_filepath)
      throw "Spotify file empty".colorize(InfoColour)
      throw :done
    else
  ...
  begin
    f.each_line do |line|
      ...
      if this_note.media_header_line.zero? 
...
instapaper_filepath = IFTTT_FILEPATH+INSTAPAPER_FILE
  ...
  elsif File.exists?(instapaper_filepath)
...
twitter_filepath = IFTTT_FILEPATH+TWITTER_FILE
  ...
  elsif File.exists?(twitter_filepath)
